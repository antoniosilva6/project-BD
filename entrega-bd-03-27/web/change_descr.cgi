...
try:
    ...
    print('<label for="descr">NEW DESCRIPTION:</label><br>')
    print('<input type="text" id="descr" name="descr">')
    ...
    description = form.getvalue('descr')
    
    if sku is not None:
        sql = 'UPDATE product SET description = %(description)s WHERE SKU = %(sku)s'
        data = {'description':description, 'sku':sku}
